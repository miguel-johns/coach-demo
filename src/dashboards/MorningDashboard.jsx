...
// ---- GENERATIVE PAYLOAD ----
// In production, this entire object is assembled by the AI job at 6:00 AM
// based on Sarah's member profile, protocol enrollment, challenge state,
// trainer schedule, and yesterday's nutrition data.
const DASHBOARD = {
  generated: "2026-04-08T06:00:12Z",
  client: { firstName: "Sarah", photo: null },
  greeting: "Good morning, Sarah.",
  subGreeting: "Wednesday, April 8 — Week 3 of Burn",
  // AI picks this based on where she is in the program + recent behavior
  ...
  protocol: {
    ...
    sessionsGoal: 9,
    totalSessions: 38,
    totalSessionsGoal: 54,
    phaseLabel: "Metabolic Conditioning",
    nextMilestone: "Week 3 complete — 1 session away",
  },

  challenge: {
    name: "28-Day Nutrition Challenge",
    day: 19,
    totalDays: 28,
    daysLogged: 17,
    streak: 5,
    mealsLoggedYesterday: 4,
    caloriesYesterday: 1820,
    calTarget: 1900,
    proteinYesterday: 138,
    proteinTarget: 140,
  },

  // Yesterday's snapshot — generative, AI decides what to highlight
  ...
  // Morning action — the one thing to do right now
  morningAction: {
    label: "Log your first meal",
    sublabel: "Tap to log breakfast or snap a photo",
    icon: "meal",
  },

  // Weekly view for context
  weekDays: [
...
function ScheduleCard({ item }) {
  ...
  return (
    <div style={{
      ...
      background: C.chip, borderRadius: 12, border: `1px solid ${C.borderLight}`,
    }}>
      <div style={{
        width: 40, height: 40, borderRadius: 10, background: C.dark, color: "#fff",
      ...
      <div style={{ fontSize: 13, fontWeight: 600, color: C.text, fontFamily: FONT, flexShrink: 0 }}>
        {item.time}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: FONT, maxWidth: 480, margin: "0 auto" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 28, fontWeight: 700, color: C.text, lineHeight: 1.2, marginBottom: 4 }}>
          {d.greeting}
        </div>
        ...
        {/* AI nudge */}
        <div style={{
          fontSize: 14, color: C.sub, lineHeight: 1.6,
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.4)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>
              {d.protocol.name} Protocol
            </div>
            <div style={{ fontSize: 18, fontWeight: 700 }}>Week {d.protocol.week} of {d.protocol.totalWeeks}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginTop: 2 }}>{d.protocol.phaseLabel}</div>
          </div>
          <ProgressRing current={d.protocol.sessionsThisWeek} goal={d.protocol.sessionsGoal}
            size={52} strokeWidth={4} color="#fff" trackColor="rgba(255,255,255,0.12)">
        ...
        <div style={{ marginBottom: 14 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 6 }}>
            <span>This week</span>
            <span>{d.protocol.nextMilestone}</span>
          </div>
          <div style={{ display: "flex", gap: 4 }}>
            {Array.from({ length: d.protocol.sessionsGoal }).map((_, i) => (
              <div key={i} style={{
                flex: 1, height: 6, borderRadius: 3,
                background: i < d.protocol.sessionsThisWeek ? "#fff" : "rgba(255,255,255,0.1)",
        ...
        <div>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "rgba(255,255,255,0.4)", marginBottom: 6 }}>
            <span>Overall</span>
            <span>{d.protocol.totalSessions}/{d.protocol.totalSessionsGoal} sessions</span>
          </div>
          <div style={{ height: 4, background: "rgba(255,255,255,0.08)", borderRadius: 2, overflow: "hidden" }}>
            <div style={{ width: `${protocolTotalPct * 100}%`, height: "100%", background: "rgba(255,255,255,0.4)", borderRadius: 2, transition: "width 0.5s ease" }} />
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 2, marginBottom: 14 }}>
          {Array.from({ length: d.challenge.totalDays }).map((_, i) => (
            <div key={i} style={{
              ...
              background: i < d.challenge.day ? (i < d.challenge.daysLogged ? C.dark : C.border) : C.borderLight,
            }} />
          ))}
        </div>
        ...
        {/* Yesterday snapshot */}
        <div style={{
          fontSize: 13, color: C.sub, lineHeight: 1.6, marginBottom: 14,
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
          {[
            ...
            <div key={s.label} style={{ background: C.bg, borderRadius: 8, padding: "10px", textAlign: "center", border: `1px solid ${C.borderLight}` }}>
              ...
              {s.target && (
                <div style={{ fontSize: 10, color: s.val >= s.target ? C.done : C.muted, fontFamily: FONT, marginTop: 1 }}>
                  / {s.target}{s.unit}
                </div>
              )}
      ...
      <div style={{
        ...
        {!mealLogged ? (
          <button onClick={() => setMealLogged(true)} style={{
            ...
            <div style={{
              ...
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                ...
                <path d="M10 7V13M7 10H13" stroke="#fff" strokeWidth="1.5" strokeLinecap="round"/>
              </svg>
            </div>
          ...
          <div style={{
            ...
            <div style={{
              width: 44, height: 44, borderRadius: 12, background: C.done,
      ...
      {/* ---- WEEK AT A GLANCE ---- */}
      <div style={{
        margin: "0 20px 16px", borderRadius: 14, padding: "18px 16px",
        background: C.chip, border: `1px solid ${C.borderLight}`,
        animation: "slideUp 0.5s ease 0.5s both",
      }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: C.muted, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 14 }}>
          This Week
        </div>
        <div style={{ display: "flex", gap: 6 }}>
          {d.weekDays.map((day, i) => {
            const hasData = day.logged || day.isToday;
            const future = !day.logged && !day.isToday;
            return (
              <div key={i} style={{ flex: 1, textAlign: "center" }}>
                {/* Day circle */}
                <div style={{
                  width: 36, height: 36, borderRadius: 18, margin: "0 auto 6px",
                  background: day.isToday ? C.dark : day.logged ? C.bg : "transparent",
                  border: day.isToday ? "none" : day.logged ? `2px solid ${C.done}` : `1px solid ${C.borderLight}`,
                  color: day.isToday ? "#fff" : day.logged ? C.done : C.muted,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  ...
                  {day.logged ? (
                    ...
                    day.scheduledSessions ? `${day.scheduledSessions}` : "—"
                  )}
                </div>
                {/* Day label */}
                <div style={{
                  fontSize: 11, fontWeight: day.isToday ? 700 : 500,
                  color: day.isToday ? C.text : day.logged ? C.sub : C.muted,
                  fontFamily: FONT,
                }}>
                  {day.label}
                </div>
                {/* Calories if logged */}
                {day.cal > 0 && (
                  <div style={{ fontSize: 10, color: C.muted, fontFamily: FONT, marginTop: 2 }}>{day.cal}</div>
                )}
              </div>
            );
          })}
      ...
      <div style={{
        ...
        Generated at 6:00 AM · April 8, 2026
      </div>
    </div>
  );
}
